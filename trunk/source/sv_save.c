...
static void ClearSaveSlot(int slot)
{
	...
	{
		snprintf(fileName, sizeof(fileName), "%shex%d%02d.hxs", SavePath, slot, i);
		remove(fileName);
	}
...
static void CopySaveSlot(int sourceSlot, int destSlot)
{
	...
	for (i = 0; i < MAX_MAPS; i++)
	{
		snprintf(sourceName, sizeof(sourceName), "%shex%d%02d.hxs", SavePath,sourceSlot, i);
		if (ExistingFile(sourceName))
		{
			snprintf(destName, sizeof(destName), "%shex%d%02d.hxs", SavePath,destSlot, i);
			CopyFile(sourceName, destName);
		}
	}
	snprintf(sourceName, sizeof(sourceName), "%shex%d.hxs", SavePath, sourceSlot);
	if (ExistingFile(sourceName))
	{
		snprintf(destName, sizeof(destName), "%shex%d.hxs", SavePath, destSlot);
		CopyFile(sourceName, destName);
	}
}
