...
int maxzone = 0x2000000;		// Maximum allocated for zone heap (8meg default)
skill_t startskill;
int startepisode;
int startmap;
BOOLEAN autostart;
...
{
"usb:/apps/hexenwii/hexen.wad",
"usb:/apps/hexenwii/hexdd.wad",
"sd:/apps/hexenwii/hexen.wad",
"sd:/apps/hexenwii/hexdd.wad",
...
void H2_DoAdvanceDemo(void)
{
	...
	case 0:
		...
		*/
		// Hermes: trick to load sound samples for menu effects
		if(one!=2)
		{
		novideo=0;
		pagetic = 100;
		...
		pagename = "TITLE";
		S_StartSongName("hexen", true);
		if(one==1) {one=2;pagetic = 8;advancedemo = 0;demosequence=-1;break;}
		}
		if(one==2)
		{
		novideo=1;
...
fixed_t FixedMul (fixed_t a, fixed_t b)
{
	...
	return (fixed_t) ((((long long) a) *((long long) b)) >> 16LL);
}
...
fixed_t FixedDiv2 (fixed_t a, fixed_t b)
{
	if (!b)
		return 0;
	return (fixed_t) (((long long) a) * ((long long)FRACUNIT) / (long long) b);
...
extern int16_t ShortSwap (int16_t x);
/*{
	return (int16_t) (((uint16_t)x << 8) | ((uint16_t)x >> 8));
}*/

extern int32_t LongSwap (int32_t x);
/*{
